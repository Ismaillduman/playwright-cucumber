...
When(
  'user enter {string} {string} {string}',
  async function (email: string, password: string, confirm_password: string) {
    const passwordForgotPage = new PasswordForgotPage(page);
    await passwordForgotPage.enterCredentials(
      email,
      password,
      confirm_password,
    );
...
Then(
  'user should see the password changed successfully message',
  async function () {
    const passwordForgotPage = new PasswordForgotPage(page);
